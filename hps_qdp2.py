...
def boston_dev():
    config = dict(
        ...
        hyper_params=dict(
            ...
            alpha=0.05,
            soften=160.,
            lambda_1=lambda t: t.suggest_discrete_uniform('lambda_1', 0.97, 0.999, 0.0002),
            lambda_2=lambda t: t.suggest_discrete_uniform('lambda_2', 0.01, 0.99, 0.01),
            ksi=10.,
            print_frequency=10,
            device='cpu'
        ),
...
def wine():
    config = dict(
        ...
        hyper_params=dict(
            ...
            aggreg_func=no_aggreg,
            hidden_size=[50, 50],
            epochs=lambda trial: int(trial.suggest_discrete_uniform('num_epoch', 100, 1000, 40)),
            batch_size=100,
            optimizer=Adam,
            learning_rate=lambda t: t.suggest_discrete_uniform('learning_rate', 0.001, 0.01, 0.001),
            scheduler=ExponentialDecay,
